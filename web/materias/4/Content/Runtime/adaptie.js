...
return oWin},showModalDialog:_(function(sUrl,vArgs,sFeatures){Adaptie.Window.showDialog(sUrl,vArgs,sFeatures,false)},"Window"),showModelessDialog:_(function(sUrl,vArgs,sFeatures){return Adaptie.Window.showDialog(sUrl,vArgs,sFeatures,false)},"Window"),setTimeout:_(function(xFunc,iDelay){if(typeof xFunc=="function"){return this.__setTimeout(function(){xFunc()},iDelay)}else{return this.__setTimeout.apply(this,arguments)}},"Window"),Scrollbars:{width:0,height:0}},Document:{createElement:_(function(sTag){var oEl,iColon=sTag.indexOf(":")
...
return aList},"HTMLDocument","HTMLElement","HTMLBodyElement","HTMLDivElement","HTMLSpanElement","HTMLFieldSetElement","HTMLQuoteElement")},Element:{uidCounter:0,appendChild:_(function(oEl){return this.insertBefore(oEl,null)},"HTMLDocument","HTMLElement","HTMLBodyElement","HTMLDivElement","HTMLSpanElement","HTMLFieldSetElement","HTMLQuoteElement"),contains:_(function(oElement){while(oElement){if(oElement==this)return true
...
sHTML=sHTML.replace(/<fieldset([^>]+?contentEditable=(["'])?true\2[^>]+)>([^<]*)<\/fieldset>/gi,"<input$1 />")
...
this.textContent=sText.replace(/\t/g," ").replace(/  /g," \u200B ")}else{Adaptie.Element.nbspify(this)}}},"HTMLElement"),outerHTML:_({get:function(){var oEl=this.ownerDocument.createElement("div")
...
oStyle.element=this
return oStyle}},"HTMLDivElement","HTMLSpanElement","HTMLFieldSetElement","HTMLInputElement","HTMLEmbedElement","HTMLImageElement","HTMLTableElement","HTMLAnchorElement","HTMLIFrameElement","HTMLQuoteElement"),tagName:_({get:function(){return this.getAttributeNS("","tagtype")||this.__tagName;}},"HTMLElement","HTMLFieldSetElement","HTMLDivElement","HTMLInputElement","HTMLQuoteElement"),uniqueID:_({get:function(){return this.__UID?this.__UID:this.__UID="emu__id"+(++Adaptie.Element.uidCounter);}},"HTMLElement"),postProcess:function(oNode,oTagProps){var aNodes=oNode.getElementsByTagName("*"),aRestore=[],aBehaviors=[],aEmbeds=[]
...
oNode.removeEventListener("emulationdone",Adaptie.Element.emulationDone,false)},fixLayout:function(oEl){if(!oEl.hasAttributeNS("","layoutfixed")){var oCurStyle=oEl.currentStyle
if(oEl.style.left&&oEl.style.left!="auto"&&oEl.style.right&&oEl.style.right!="auto"){oEl.style.right="auto"}
if(oEl.style.top&&oEl.style.top!="auto"&&oEl.style.bottom&&oEl.style.bottom!="auto"){oEl.style.bottom="auto"}
...
var sW=oEl.pctWidth
if(sW&&oEl.nodeName=="FIELDSET"&&oEl.offsetParent){var pLeft=parseInt(oEl.parentNode.currentStyle.paddingLeft),pRight=parseInt(oEl.parentNode.currentStyle.paddingRight),bLeft=parseInt(oEl.parentNode.currentStyle.borderLeftWidth),bRight=parseInt(oEl.parentNode.currentStyle.borderRightWidth),mLeft=0,mRight=0
if(oCurStyle.position!="absolute"){mLeft=parseInt(oCurStyle.marginLeft)
mRight=parseInt(oCurStyle.marginRight)}
oEl.parentNode.setAttributeNS("","nomargins","1")
var pW=oEl.parentNode.offsetWidth-pLeft-pRight-bLeft-bRight-mLeft-mRight
oEl.style.width=pW*(parseFloat(sW)/100)
...
oEl.removeAttributeNS("","nomargins")},fixOverflowClosure:function(oEl,sProp){return function(){Adaptie.Element.fixOverflow(oEl,sProp)}},fixRenderBugs:function(oEl){if(!Adaptie.HasInlineBlock){if(oEl.currentStyle.position=="absolute"){Adaptie.Element.fixOffsetParent(oEl)}else{Adaptie.Element.fixRenderOrder(oEl)}}},fixRenderOrder:function(oEl){var oParent=oEl.parentNode
...
if(oEl.offsetTop==-2000&&oEl.curTop&&pTop>0){oEl.style.top=oEl.curTop
...
oEl.style.right="auto"}}}},nbspify:function(oEl){oEl.textContent="\xA0"
...
this[prop]=this.getPropertyValue(prop)}},cssText:_({set:function(sCSS){sCSS=sCSS||""
sCSS=Adaptie.Style.Rules.processCursor(sCSS)
sCSS=sCSS.replace(/url\((?!&quot;)([\"\'])?(.+?)\1\)/gi,'url("$2")')
sCSS=sCSS.replace(/font-family\s*:\s*(?!&quot;)([\"\'])?(.+?)\1(;|$)/gi,'font-family:"$2"$3')
this.__cssText=sCSS
if(this.element)Adaptie.Element.fixZIndex(this.element)}},"CSSStyleDeclaration"),cursor:_({get:function(){var sCursor=this.getPropertyValue("cursor")
return sCursor=="pointer"?"hand":sCursor},set:function(sCursor){if(sCursor=="hand")sCursor="pointer"
this.setProperty("cursor",sCursor,"")}},"CSSStyleDeclaration","ComputedCSSStyleDeclaration"),display:_({get:function(){var sDisp=this.getPropertyValue("display");return sDisp=="table-caption"||sDisp=="inline-block"?"inline":sDisp;},set:function(sDisp){sDisp=sDisp.toLowerCase()
...
Adaptie.Element.fitParentsHeight(this.element)}}},"CSSStyleDeclaration","ComputedCSSStyleDeclaration"),filter:_({get:function(){if(this.opacity){return"alpha(opacity="+this.opacity*100+")"}
return""},set:function(sFilter){if(sFilter){if(/(?:progid:DXImageTransform\.Microsoft\.)?alpha\(opacity=([\d\.]+)\)/.test(sFilter)){this.opacity=RegExp.$1/100}}else{this.opacity=""}}},"CSSStyleDeclaration"),hasLayout:_({get:function(){return true;}},"ComputedCSSStyleDeclaration"),height:_({get:function(){return this.getPropertyValue("height");},set:function(sVal){this.setProperty("height",sVal,"")
if(this.element){delete this.element.__curHeight
delete this.element.__origHeight
if(sVal!="auto"){Adaptie.Element.fitHeightToContent(this.element)}}}},"CSSStyleDeclaration"),opacity:_({get:function(){return this.getPropertyValue("opacity");},set:function(sOpacity){var iCur=this.element.currentStyle.opacity,iNew=+sOpacity,bParentFix=iCur==1&&iNew!=1||iCur!=1&&iNew==1
...
sCSS=sCSS.replace(/url\([^\)]*[\{\}][^\)]*\)/g,function(sUrl){return sUrl.replace(/\{/g,"%7B").replace(/\}/g,"%7D")})
...
if(/[\s;{](?:width\s*:\s*([\d\.]+)%)/i.test(sRuleText))iPctWidth=Math.round(RegExp.$1)
if(/[\s;{](?:height\s*:\s*([\d\.]+)%)/i.test(sRuleText))iPctHeight=Math.round(RegExp.$1)
...
sRuleText=sRuleText.replace(/url\((?!&quot;)([\"\'])?(.+?)\1\)/gi,function(sAll,sQuote,sUrl){return"url(&quot;"+sUrl.replace(/&/g,"&amp;")+"&quot;)"})
...
sRuleText=sRuleText.replace(/font-family\s*:\s*(?!&quot;)([\"\'])?(.+?)\1(;|$)/gi,"font-family:&quot;$2&quot;"+(oParent?","+oParent.currentStyle.fontFamily.replace(/\"/g,"&quot;"):"")+"$3")
