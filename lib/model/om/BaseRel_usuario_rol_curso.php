...
abstract class BaseRel_usuario_rol_curso extends BaseObject  implements Persistent {
	...
	protected $fecha_primer_conex;


	protected $fecha_ultima_conex;
	...
	public function getFechaPrimerConex($format = 'Y-m-d H:i:s')
	{

		if ($this->fecha_primer_conex === null || $this->fecha_primer_conex === '') {
			return null;
		} elseif (!is_int($this->fecha_primer_conex)) {
						$ts = strtotime($this->fecha_primer_conex);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse value of [fecha_primer_conex] as date/time value: " . var_export($this->fecha_primer_conex, true));
			}
		} else {
			$ts = $this->fecha_primer_conex;
		}
		if ($format === null) {
			return $ts;
		} elseif (strpos($format, '%') !== false) {
	...
	public function getFechaUltimaConex($format = 'Y-m-d H:i:s')
	{

		if ($this->fecha_ultima_conex === null || $this->fecha_ultima_conex === '') {
			return null;
		} elseif (!is_int($this->fecha_ultima_conex)) {
						$ts = strtotime($this->fecha_ultima_conex);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse value of [fecha_ultima_conex] as date/time value: " . var_export($this->fecha_ultima_conex, true));
			}
		} else {
			$ts = $this->fecha_ultima_conex;
		}
		if ($format === null) {
			return $ts;
		} elseif (strpos($format, '%') !== false) {
	...
	public function setFechaPrimerConex($v)
	{

		if ($v !== null && !is_int($v)) {
			$ts = strtotime($v);
			if ($ts === -1 || $ts === false) { 			throw new PropelException("Unable to parse date/time value for [fecha_primer_conex] from input: " . var_export($v, true));
			}
		} else {
			$ts = $v;
		}
		if ($this->fecha_primer_conex !== $ts) {
			$this->fecha_primer_conex = $ts;
			$this->modifiedColumns[] = Rel_usuario_rol_cursoPeer::FECHA_PRIMER_CONEX;
		}

	} 


	public function setFechaUltimaConex($v)
	{

		if ($v !== null && !is_int($v)) {
			$ts = strtotime($v);
			if ($ts === -1 || $ts === false) { 			throw new PropelException("Unable to parse date/time value for [fecha_ultima_conex] from input: " . var_export($v, true));
			}
		} else {
			$ts = $v;
		}
		if ($this->fecha_ultima_conex !== $ts) {
			$this->fecha_ultima_conex = $ts;
			$this->modifiedColumns[] = Rel_usuario_rol_cursoPeer::FECHA_ULTIMA_CONEX;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->fecha_primer_conex = $rs->getTimestamp($startcol + 8, null);

			$this->fecha_ultima_conex = $rs->getTimestamp($startcol + 9, null);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 7:
				...
				break;
			case 8:
				return $this->getFechaPrimerConex;
				break;
			case 9:
				return $this->getFechaUltimaConex;
				break;
			default:
				return null;
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[8] => $this->getFechaPrimerConex(),
			$keys[9] => $this->getFechaUltimaConex(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 7:
				...
				break;
			case 8:
				$this->setFechaPrimerConex($value);
				break;
			case 9:
				$this->setFechaUltimaConex($value);
				break;
		} 	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[8], $arr)) $this->setFechaPrimerConex($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setFechaUltimaConex($arr[$keys[9]]);
	}
	...
	if ($this->isColumnModified(Rel_usuario_rol_cursoPeer::FECHA_PRIMER_CONEX)) $criteria->add(Rel_usuario_rol_cursoPeer::FECHA_PRIMER_CONEX, $this->fecha_primer_conex);
	if ($this->isColumnModified(Rel_usuario_rol_cursoPeer::FECHA_ULTIMA_CONEX)) $criteria->add(Rel_usuario_rol_cursoPeer::FECHA_ULTIMA_CONEX, $this->fecha_ultima_conex);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setFechaPrimerConex($this->fecha_primer_conex);

		$copyObj->setFechaUltimaConex($this->fecha_ultima_conex);
