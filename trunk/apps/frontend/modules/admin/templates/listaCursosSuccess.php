...
<div id="divadmin">
  <div class="tit_box_mensajes"><h2 class="titbox">Lista de cursos para el <?php echo $rol." ".$usuario->getNombre()." ".$usuario->getApellidos()?> </h2></div>
  <div class="cont_box_correo">
					  ...
					  <td class="td3"><?php if (!$rels) : ?>
					           <?php if ($rol=="alumno") : ?>
					            <?php if ($curso->getCurso()->esMoroso($usuario->getId())) : ?>
                      				  ...
                      				  <?php endif; ?>
                      			<?php endif; ?>
					            <?php echo link_to('Eliminar','admin/eliminarCursoUsuario?idusuario='.$usuario->getId().'&rol='.$rol.'&idcurso='.$curso->getCurso()->getId(),'confirm=&iquest;Esta seguro que desea eliminar el '.$curso->getCurso()->getNombre().' ?') ?>
						   <?php endif; ?>
					  </td>
                  </tr>
                  <?php $i++; ?>
    ...
    <br>
    <?php echoNotaInformativa('Ayuda', "Desde este panel tendr&aacute; acceso a la lista de cursos matriculados por el usuario <b>".$usuario->getNombre()." ".$usuario->getApellidos()."</b> y tendr&aacute; la posibilidad de baja al usuario del curso como limitarle el acceso temporalmente hasta que realize los pagos."); ?>
    <br>
    <?php echoWarning('Aviso', "Si selecciona 'suspender' en alg&uacute;n curso al usuario <b>".$usuario->getNombre()." ".$usuario->getApellidos()."</b> dejar&aacute; de tener acceso al curso hasta que lo vuelva 'reanudar'"); ?>
    <br>
    <?php echoWarning('Aviso', "Solo podr&aacute 'suspender' en los cursos que no pertenezcan a ning&uacute;n m&oacute;dulo"); ?>
