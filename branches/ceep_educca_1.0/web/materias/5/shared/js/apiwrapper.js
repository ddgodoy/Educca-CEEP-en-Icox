...
if (navigator.appName && navigator.appName.indexOf("Microsoft") != -1 &&
	...
	document.write('Sub 1_FSCommand(ByVal command, ByVal args)\n');
	document.write('  call 1_DoFSCommand(command, args)\n');
...
<!--
// FS SCORM - fscommand adapter for ADL SCORM 1.2
// version 1.2.4    06/25/03
...
function planempresa_DoFSCommand(command, args){
  var planempresaObj = g_bIsInternetExplorer ? planempresa : document.planempresa;
  ...
  if (!APIOK()) return;
	if (cmd.substring(0,3) == "LMS"){
		...
		if ( cmd == "LMSInitialize" ){
			...
			if ( cmd == "LMSGetValue") {
				planempresaObj.SetVariable(arg2, SCOGetValue(arg1));
			}	else if ( cmd == "LMSGetLastError") {
				planempresaObj.SetVariable(arg2, SCOGetLastError(arg1));
			}	else if ( cmd == "LMSGetErrorString") {
				planempresaObj.SetVariable(arg2, SCOGetLastError(arg1));
			}	else if ( cmd == "LMSGetDiagnostic") {
				planempresaObj.SetVariable(arg2, SCOGetDiagnostic(arg1));
			}	else {
          // for unknown LMSGetxxxx extension
         v = eval('g_objAPI.' + cmd + '(\"' + arg1 + '\")');
         planempresaObj.SetVariable(arg2,v);
			}
      ...
      err = SCOSetValue("cmi.core.exit", "time-out")
		...
		if ((cmd!="CMIInitialize")&&(cmd!="CMIFinish")&&(cmd!="CMIExitAU")&&(cmd!="MM_StartSession")){
