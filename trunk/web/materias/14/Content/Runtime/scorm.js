/*@cc_on @*/
;/*@if(@debugMode)
var LMSDebug={Init:function(){this.debugWin=window.open("about:blank","debugWin","menu=0,location=0,width=500,height=700,scrollbars=1,resizable=1")
this.doc=this.debugWin.document
this.doc.write('<h1>LMS Debug - SCORM 1.2</h1><ol id=debugList style="white-space:nowrap;font-family:arial;font-size:10pt"></ol>')},Errors:function(){var sErr=LMSInterface.API.LMSGetLastError(),sErrStr="Unknown error",sErrDiag="No diagnostic"
...
LMSDebug.Out("Error","LMS error details: #"+sErr+", "+sErrStr+", "+sErrDiag)}},OpColors:{Info:"black",Error:"red",GetValue:"blue",SetValue:"green"},Queue:[],Line:function(op,msg){var logLine=this.debugWin.debugList.appendChild(this.doc.createElement("LI"))
logLine.innerHTML='<strong style="color:'+(this.OpColors[op]||"purple")+'">'+op+": </strong>"
logLine.appendChild(this.doc.createTextNode(msg))},Out:function(op,msg,giveUp){if(this.debugWin&&!this.debugWin.closed){if(!this.debugWin.debugList){setTimeout(function(){LMSDebug.Out();},100)
if(msg)this.Queue.unshift({op:op,msg:msg})
return}
for(var i=this.Queue.length;i--;)this.Line(this.Queue[i].op,this.Queue[i].msg)
this.Queue.length=0
if(msg)this.Line(op,msg)
this.doc.body.scrollTop=this.doc.body.scrollHeight}else{if(!giveUp){this.Init()
this.Out(op,msg,true)}else{alert("Debug window couldn't be initialized")}}}}
;/*@end @*/
...
if(this.API){;/*@if(@debugMode)LMSDebug.Out("Info","SCORM API found, calling initialize");/*@end @*/
try{var bInit=this.API.LMSInitialize("")
;/*@if(@debugMode)
LMSDebug.Out("Initialize","LMSInitialize result was: "+bInit)
LMSDebug.Errors()
;/*@end @*/
...
try{if(parent&&!parent.API)parent.API=window.API;}catch(e){}}catch(oErr){;/*@if(@debugMode)LMSDebug.Out("Error","LMSInitialize threw an exception, turning off SCORM");/*@end @*/
...
this.API=null}}else{;/*@if(@debugMode)LMSDebug.Out("Info","SCORM API not found");/*@end @*/}
...
var oTypes={dragdrop:"matching",fillblank:"fill-in",multichoice:"choice",rating:"numeric",text:"fill-in"}
...
LMSDebug.Errors()
;/*@end @*/
return bSuccess&&LMSInterface.API.LMSGetLastError()==0},GetUserId:function(){return this.GetVariable("cmi.core.student_id")},Dispose:function(){if(!LMSInterface.API&&!LMSInterface.Track)return null
...
if(LMSInterface.API){;/*@if(@debugMode)LMSDebug.Out("Info","Done, calling commit and finish");/*@end @*/
var bCmt=LMSInterface.API.LMSCommit("")
;/*@if(@debugMode)
LMSDebug.Out("Commit","LMSCommit result was: "+bCmt)
LMSDebug.Errors()
;/*@end @*/
var bFin=LMSInterface.API.LMSFinish("")
;/*@if(@debugMode)
LMSDebug.Out("Finish","LMSFinish result was: "+bFin)
LMSDebug.Errors()
;/*@end @*/}}}