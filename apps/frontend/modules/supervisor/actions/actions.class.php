...
//
  ...
  public function executeInformeTripartita()
  {
    ...
    $results = Rel_paquete_cursoPeer::doSelectOne($c2);
    $idPaquete = $results->getIdPaquete();

    $c3 = new Criteria();
    $c3->add(PaquetePeer::ID,$idPaquete);
    $results2 = PaquetePeer::doSelectOne($c3);

    $modulo = $results2->getNombre();
   // $paqueteCurso = Rel_paquete_cursoPeer::RetrieveByPk($results->getPaqueteId());
    ...
    $pdf->SetTitle("Informe de evaluacion tripartita, ".$modulo.$curso->getNombre());
    $pdf->SetSubject("Informe de evaluacion tripartita");
    $pdf->SetKeywords("informe evaluacion tripartita ".$modulo.$curso->getNombre());
   ...
   // $pdf->Cell(15, 6, "FUC", 1, 0, 'C', 1);
    ...
    foreach($alumnos as $alumno)
    {
      ...
      $pdf->Cell(70, 6, $texto_tiempo, 'LR', 0, 'L', $fill);
      $pdf->Cell(90, 6, "Propuestos: $_cant_en_total   Realizados: $_cant_realizados  Porcentaje: ".number_format($_val_porcentaje,2)."%   Nota media: ".number_format($_val_notamedia,2), 'LR', 0, 'L', $fill);
