...
class sfMyBrowser
{
  ...
  { $dom = $this->getResponseDom();
    ...
    // text link
    //if ($link = $xpath->query(sprintf('//a[.="%s"]', $name))->item(0)) no funciona
    //if ($link = $xpath->query(sprintf('//a[@id="%s"]', $name))->item(0))
    if ($link = $xpath->query(sprintf('//a[@id="%s" or @name="%s"]', $name, $name))->item(0))
    ...
    if (!$form = $xpath->query(sprintf('//input[((@type="submit" or @type="button") and (@value="%s" or @id="%s") ) ]/ancestor::form', $name, $name))->item(0))
    {
      ...
      if ($nodeName == 'input' && ($element->getAttribute('type') == 'checkbox' || $element->getAttribute('type') == 'radio'))
      {
        if ($element->getAttribute('checked'))
        {
          $value = $element->getAttribute('value');
             ...
             {    $filename = $this->fields[$elementName];
                  ...
                  $trozos = explode(".", basename($filename));
                  switch($trozos[1]){
                    case "zip": $type='application/zip'; break;
                    case "jpg": $type='image/jpg'; break;
                    case "jpeg": $type='image/jpeg'; break;
                    case "pjpeg": $type='image/pjpeg'; break;
                    default: $type=null; break;
                  } // switch
       		        $this->files[$elementName] = array('name' => basename($filename), 'type' => $type, 'tmp_name' => $filename, 'error' => $fileError, 'size' => $fileSize);
             }
 		      }
      else if (
        $nodeName == 'input'
