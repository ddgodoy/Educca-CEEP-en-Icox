...
{
  /**
   * executeIndex(), el index debe mostrar en portada información relevante
   * sobre los alumnos
   * Autor: Todor Todorov
   */
  ...
  /**
   *  Nombre del método: executeMaterias()
   * Autor: Jacobo Chaquet
   * Descripción: Lista todos las materias
   */
  ...
  /*
   *  Nombre del método: executeEjercicios()
   *  Autor: Ángel Martín
   *  Descripción: Accede a la gestión de ejercicios de administrador
   */
  ...
  /*
   *  Nombre del método: executeListarEjercicios()
   *  Autor: Ángel Martín
   *  Descripción: Lista ejercicios para administracion
   */
  ...
  /*
   *  Nombre del método: executeImportarEjercicios()
   *  Autor: Ángel Martín
   *  Descripción: Lista ejercicios para administracion
   */
  ...
  // Nombre del método: executeUsuarios()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Lista todos los usuarios confirmados de la plataforma*/
  ...
  public function executeUsuarios()
  {
    ...
    { $this->superUsuario = '1';
      $usuario = new Usuario();
      $usuarios = $usuario->getSuperUsuarios(null,$c);
      $arrayUsuarios = array();
      foreach($usuarios as  $usuario)
      { $arrayUsuarios[]= $usuario->getUsuario();
      }
  ...
  // Nombre del método: executeNuevoCurso()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Muestra el formulario de creación de curso
                  o bien inserta e inicializa el curso dado*/
        ...
        $con = Propel::getConnection();
  			try
	            ...
	            {  $notifacacion = new Notificacion();
    	           $notifacacion->setInfo($administrador->getId(),$curso->getId(),'Nuevo Curso','Creaci&oacute;n del curso: <b>'.$curso->getNombre().'</b> por '.$this->getUser()->getAdministrador(),date("Y-m-d H:j"));
    	        }
    					$con->commit();
          }
  			catch (Exception $e)
 ...
 */
  ...
  // Nombre del método: executeImagenCurso()
  // Añadida por: Jacobo Chaquet
  /* Descripción: guarda una imagen del curso para el cartel comercial */
    ...
    if ($this->getRequest()->getMethod() == sfRequest::POST)
    {
          ...
          { $thumbnail = new sfThumbnail(253, 550,false,false,95);
            ...
            $administradores= $this->getUser()->getAdministradores();
      	     foreach ($administradores as $administrador)
       	     {  $notifacacion = new Notificacion();
          	    $notifacacion->setInfo($administrador->getId(),$this->curso->getId(),'Nueva imagen Curso','Eliminada imagen del curso: <b>'.$this->curso->getNombre().'</b> por '.$this->getUser()->getAdministrador(),date("Y-m-d H:i"));
          	 }
  ...
  // Nombre del método: executEliminarImagenCurso()
  // Añadida por: Jacobo Chaquet
  /* Descripción: elimina la imagen del curso*/
  ...
  { if (!$this->getRequestParameter('idcurso'))
    ...
    if (file_exists(sfConfig::get('sf_web_dir').DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'img_cursos'.DIRECTORY_SEPARATOR.'portada_'.$curso->getId().'.jpg'))
    {
       ...
       $administradores= $this->getUser()->getAdministradores();
	     foreach ($administradores as $administrador)
	     {  $notifacacion = new Notificacion();
    	    $notifacacion->setInfo($administrador->getId(),$curso->getId(),'Eliminaci&oacute;n imagen Curso','Eliminada imagen del curso: <b>'.$curso->getNombre().'</b> por '.$this->getUser()->getAdministrador(),date("Y-m-d H:j"));
    	 }
  ...
  public function executeProfesores()
  {
      ...
      $c = new Criteria();
      $c->addAscendingOrderByColumn(UsuarioPeer::APELLIDOS);
      $this->profesores = $usuario -> getProfesores(null,$c);
      return;
  }


  // Nombre del método: executeModulos()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Lista todos los paquetes
   */
  ...
  // Nombre del método: executeNuevoUsuario()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Muestra el formulario de creación de un usuario (profesor/alumno)
  ...
  public function executeNuevoUsuario()
  {
      ...
      $con = Propel::getConnection();
  			try
           	  ...
           	  $usuario->emailUsuario(null,3,'confirmacion'); //genera tb el password
           		/*if ( ("administrador"==$this->rol) || ("supervisor"==$this->rol) || ("profesor"==$this->rol))
           		{*/
              ...
              foreach ($administradores as $administrador)
      	            {  $notifacacion = new Notificacion();
          	           $notifacacion->setInfo($administrador->getId(),null,'Nuevo '.$this->rol,'Nuevo '.$this->rol.' '.$usuario->getNombre().' '.$usuario->getApellidos().' creado por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"),$tipo);
          	        }
              ...
              }  else	{ if ("alumno"==$this->rol)
                        {  $c->add(RolPeer::NOMBRE, "alumno");
                        } else { if ("administrador"==$this->rol)
                                 {  $c->add(RolPeer::NOMBRE, "administrador");
                                 } else { if ("supervisor"==$this->rol)
              ...
              if ( ("administrador"==$this->rol) || ("supervisor"==$this->rol))
           		{
           		  $c = new Criteria();
     ...
     $this->mostrarForm='No'; // para que en el template no vuelva ha salir el formulario
     $this->usuario = $usuario; // necesario para test
  ...
  // Nombre del método: executeNuevoModulo()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Muestra el formulario de creación de modulo
    			  o bien inserta e inicializa el modulo dado
   */
          ...
          $j=0;
	        ...
	        for ( $i=0;$i<$total;$i++ )
    				   ...
    				   if ( $micurso )
               {
    				    $curso= CursoPeer::retrieveByPk($micurso);
    						$this->forward404Unless($curso);
    						//echo "alta en curso:".$curso->getNombre()."<br>";
    						if (!isset($diaInicioPaquete)) {
    							...
    							/*$webcam= $curso->getWebcam();*/
    							$scan= $curso->getScan();
    						   }
    						else{  /*$webcam += $curso->getWebcam();*/
    							     $scan += $curso->getScan();
      	...
      	if (isset($diaInicioPaquete))
      	{
              $con = Propel::getConnection();
        			try
                 	...
                 	$modalidad = $this->getRequestParameter('modalidad');
                 	if ($modalidad == 'mensual') {$modulo->setMensual(1);}
                 	else {$modulo->setMensual(0);}
                 	/*$modulo->setWebcam($webcam);*/
                 	$modulo->setScan($scan);
                 ...
                 $administradores= $this->getUser()->getAdministradores();
      				   foreach ($administradores as $administrador)
      	         {  $notifacacion = new Notificacion();
          	        $notifacacion->setInfo($administrador->getId(),null,'Nuevo m&oacute;dulo','Nuevo m&oacute;dulo '.$modulo->getNombre().' creado por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
          	     }
          			 $con->commit();
                }
        			catch (Exception $e)
  ...
  // Nombre del método: actualizaModulos()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Si se modifica un curso, mira en que modulos esta y actualiza si hace falta las fechas, webcam, y scan
   */
  ...
  // Nombre del método: executeModificarCurso()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Modifica la informacion de un curso
   */
      ...
      $idcurso = $this->getRequestParameter('idcurso');
	     ...
	     if ($this->getRequest()->getMethod() == sfRequest::POST)
       {
          $con = Propel::getConnection();
             ...
             $modalidad = $this->getRequestParameter('modalidad');
             if ($modalidad == 'mensual') {$this->curso->setMensual(1);}
             else {$this->curso->setMensual(0);}
             $this->curso->save($con);

             $administradores= $this->getUser()->getAdministradores();
      	     foreach ($administradores as $administrador)
       	     {  $notifacacion = new Notificacion();
          	    $notifacacion->setInfo($administrador->getId(),$this->curso->getId(),'Modifiaci&oacute;n Curso','Modificado curso: <b>'.$this->curso->getNombre().'</b> por '.$this->getUser()->getAdministrador(),date("Y-m-d H:j"));
          	 }
  ...
  // Nombre del método: executeModificarModulo()
  // Añadida por: Jacobo Chaquet
  // Descripción: Modifica la informacion de un curso
          ...
          $j=0;
           ...
           $con = Propel::getConnection();
  			   try
            {
                ...
                foreach ($administradores as $administrador)
                {  $notifacacion = new Notificacion();
                   $notifacacion->setInfo($administrador->getId(),null,'M&oacute;dulo modificado','M&oacute;dulo modificado '.$this->modulo->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
                }
          			$con->commit();
            }
  			   catch (Exception $e)
  ...
  // Nombre del método: executeListaCursos()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Muestra los cursos en que esta matriculado un usuario
   */
  ...
  // Nombre del método: executeListaModulos()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Muestra los modulos en que esta matriculado un usuario
   */
  ...
  public function executeEliminarUsuario()
  {
    ...
    foreach ($administradores as $administrador)
    {  $notifacacion = new Notificacion();
       $notifacacion->setInfo($administrador->getId(),null,'Eliminado usuario','Eliminado usuario '.$usuario->getNombre().' '.$usuario->getApellidos().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
    }
    ...
    if ("profesor" == $rol)
    {
      $this->redirect('admin/profesores');
           ...
           else{  if ($this->getRequestParameter('superUsuario'))
                  {
                     $this->redirect('admin/usuarios?superUsuario=1');
                  } else $this->redirect('admin/usuarios');
                }
          }
  }
 ...
 */
  ...
  public function executeConfirmarUsuario()
  {
    ...
    foreach ($administradores as $administrador)
    {  $notifacacion = new Notificacion();
       $notifacacion->setInfo($administrador->getId(),null,'Usuario confirmado '.$usuario->getNombreusuario(),'Usuario confirmado '.$usuario->getNombre().' '.$usuario->getApellidos().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
    }
  ...
  // Nombre del método: eliminarCurso()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Elimina un curso si no tiene alumnos
   */
  ...
  public function executeEliminarCurso()
  {
     ...
     if ($this->curso->vacio())
     {
         ...
         foreach ($administradores as $administrador)
	            {  $notifacacion = new Notificacion();
    	           $notifacacion->setInfo($administrador->getId(),null,'Curso Eliminado','Curso borrado: <b>'.$this->curso->getNombre().'</b> por '.$this->getUser()->getAdministrador(),date("Y-m-d H:j"));
    	        }
         $this->curso->delete();
  ...
  // Nombre del método: eliminarModulo()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Elimina un modulo si no tiene alumnos
   */
  ...
  public function executeEliminarModulo()
  {
   ...
   if ($this->modulo->estaVacio())
   {
      ...
      foreach ($administradores as $administrador)
      {  $notifacacion = new Notificacion();
         $notifacacion->setInfo($administrador->getId(),null,'M&oacute;dulo eliminado','M&oacute;dulo eliminado'.$this->modulo->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
      }
  ...
  // Nombre del método: executeEliminarCursoUsuario()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Elimina un curso a un usuario
   */
  public function executeEliminarCursoUsuario()
  {
     ...
     foreach ($administradores as $administrador)
     {  $notifacacion = new Notificacion();
        $notifacacion->setInfo($administrador->getId(),$idcurso,'Eliminado '.$rol.' '.$usuario->getNombreusuario().' de curso','Eliminado '.$rol.' '.$usuario->getNombre().' '.$usuario->getApellidos().' del curso '.$curso->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
     }

     $this->redirect('admin/listaCursos?idusuario='.$idusuario.'&rol='.$rol);
  }

 // Nombre del método: eliminarModuloUsuario()
 // Añadida por: Jacobo Chaquet
 /* Descripción: Elimina un curso a un usuario
 */
  ...
  public function executeEliminarModuloUsuario()
  {
     ...
     try
            ...
            foreach ($administradores as $administrador)
            {  $notifacacion = new Notificacion();
               $notifacacion->setInfo($administrador->getId(),null,'Eliminado '.$usuario->getNombreusuario().' de m&oacute;dulo','Eliminado usuario '.$usuario->getNombre().' '.$usuario->getApellidos().' del m&oacute;dulo '.$modulo->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
            }
  				 $con->commit();
  			 }
    	catch (Exception $e)
  ...
  // Nombre del método: AniadirCurso()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Aniade curso
   */
              ...
              $micurso = $this->getRequestParameter("cursos$i");
				   		if ( $micurso )
               {
				        	$curso= CursoPeer::retrieveByPk($micurso);
    							...
    							$relacion->save($con);
    							//echo "dado de alta usuario:".$idusuario."  rol:".$id_rol."curso".$micurso."  ".$curso->getNombre();

    							$administradores= $this->getUser()->getAdministradores();
                  foreach ($administradores as $administrador)
                  {  $notifacacion = new Notificacion();
                     $notifacacion->setInfo($administrador->getId(),$micurso,'A&ntilde;adido '.$this->usuario->getNombreusuario().' a curso','A&ntilde;adido '.$this->rol.' '.$this->usuario->getNombre().' '.$this->usuario->getApellidos().' al curso '.$curso->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
                  }
					      }
				     }
				     $con->commit();
 ...
 // Nombre del método: AniadirModulo()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Aniade curso
   */
          ...
          $this->idusuario=$idusuario;
		      ...
		      $this->errores ="";
				   		 ...
				   		 if ( $mimodulo )
                {
                  ...
                  foreach ($administradores as $administrador)
                  {  $notifacacion = new Notificacion();
                     $notifacacion->setInfo($administrador->getId(),null,'A&ntilde;adido m&oacute;dulo a '.$this->usuario->getNombreusuario(),'A&ntilde;adido '.$this->usuario->getNombre().' '.$this->usuario->getApellidos().' al m&oacute;dulo '.$modulo->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
                  }
                }
				     }
				     $con->commit();
  ...
  // Nombre del método: executeListaCursosModulo()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Muestra los cursos de un modulo
   */
  ...
  // Nombre del método: executeMoroso()
  // Añadida por: Jacobo Chaquet
  /* Descripción: cambia el estado de moroso a No moroso y viceversa
   */
   	...
   	try
		{
     		 ...
     		 if ($moroso == 'no')
          {
             ...
             foreach ($administradores as $administrador)
      	            {  $notifacacion = new Notificacion();
          	           $notifacacion->setInfo($administrador->getId(),$idcurso,'Reanudado el acceso '.$this->usuario->getNombreusuario(),'Reanudado el acceso a '.$this->usuario->getNombre().' '.$this->usuario->getApellidos().' en el curso '.$this->curso->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
          	        }
     			...
     			}else { if ($moroso == 'si')
                  {
                    ...
                    $c->add(RolPeer::NOMBRE, "moroso");
                     ...
                     foreach ($administradores as $administrador)
              	            {  $notifacacion = new Notificacion();
                  	           $notifacacion->setInfo($administrador->getId(),$idcurso,'Prohibido el acceso '.$this->usuario->getNombreusuario(),'Prohibido el acceso a '.$this->usuario->getNombre().' '.$this->usuario->getApellidos().' en el curso '.$this->curso->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
                  	        }
                  }
  				     }
     		 $con->commit();
  ...
  // Nombre del método: executeMorosoModulo()
  // Añadida por: Jacobo Chaquet
  /* Descripción: cambia el estado de moroso a No moroso y viceversa de un usuario en un modulo
   */
     ...
     try
     {
     		...
     		if ($moroso == 'no')
        {
            ...
            $administradores= $this->getUser()->getAdministradores();
             foreach ($administradores as $administrador)
      	            {  $notifacacion = new Notificacion();
          	           $notifacacion->setInfo($administrador->getId(),null,'Reanudado el acceso a '.$this->usuario->getNombreusuario(),'Reanudado el acceso a '.$this->usuario->getNombre().' '.$this->usuario->getApellidos().' en el m&oacute;dulo '.$this->paquete->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
          	        }
        }
     		else {  if ($moroso == 'si')
                {
                  ...
                  foreach ($administradores as $administrador)
      	            {  $notifacacion = new Notificacion();
          	           $notifacacion->setInfo($administrador->getId(),null,'Prohibido el acceso a '.$this->usuario->getNombreusuario(),'Prohibido el acceso a '.$this->usuario->getNombre().' '.$this->usuario->getApellidos().' en el m&oacute;dulo '.$this->paquete->getNombre().' por administrador <b>'.$this->getUser()->getAdministrador().'</b>',date("Y-m-d H:j"));
          	        }
                }
  			     }
  ...
  // Nombre del método: executeBuscar()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Busca usuarios
   */
...
function unzip($src_file, $dest_dir=false, $create_zip_name_dir=true, $overwrite=true)
{
  ...
  // Nombre del método: executeEditMateria()
  // Creada por: Ángel Martín
  // Descripción: crea/edita una materia

  public function executeEditMateria()
  {
    //$df= fopen("C:/env_file.txt",'w');
    ...
    // Si se ha hecho submit procesamos el formulario
    {
      ...
      if (!$nerrores)
      {
        // Iniciamos los parámetros para transacciones seguras con la base de datos
        $con = Propel::getConnection();
        $fallo_bbdd = 0;
        ...
        try
        {
          ...
          {
            $notifacacion = new Notificacion();
	          $notifacacion->setInfo($administrador->getId(),null,$tipo.' materia',$tipo.' materia: <b>'.$materia->getNombre().'</b> por '.$this->getUser()->getAdministrador(),date("Y-m-d H:j"));
	        }
        ...
        //fwrite($df,"\n\n".$fallo_bbdd."  ".$_FILES['my_file']['tmp_name']);
        if ((!$fallo_bbdd) && (is_readable($_FILES['my_file']['tmp_name'])))
        // Si la consulta se realizó con éxito y se subió un fichero...
        {
          ...
          self::borrarDirectorio($whereToUnzip);

          //$this->getRequest()->moveFile('my_file',  $zipFile);
          ...
          if ($zip)
          {
            ...
            // Procesamiento de objetos SCO tanto para cursos SCORM como para Composica
            {
              ...
              if ($scormh->getLastError()) {return;}
              $scormh->loadScos();
              if ($scormh->getLastError() > 50) {return;}
            }
  ...
  // Nombre del método: executeContenidoMateria()
  // Creada por: Ángel Martín
  // Descripción: En caso de contenidos segmentados (no scorm)
  // esta función explora la estructura de directorios de la materia
  // y encuentra las páginas que podrán ser usadas como índice de cada
  // tema
  ...
  public function executeContenidoMateria()
  {
    ...
    // Procesamos el formulario
    {
      ...
      // Borramos la asignación de temas anterior
      $c = new Criteria();
    ...
    // Llegamos a esta página desde la ficha de la materia
    {
      $id = $this->getRequestParameter('idmateria');
      $materia = MateriaPeer::RetrieveByPk($id);
  ...
  private function procesarDirectorio( $path,$dir,$materiaId )
  {
    if (is_dir($path.DIRECTORY_SEPARATOR.$dir))
    {
      ...
      if ($gd = opendir( $dirAux))
      {
          ...
          {
              //echo "nombre de archivo: $dirAux $archivo : tipo de archivo: " . filetype( $dirAux . $archivo) . "<br>\n";
...
private function borrarDirectorio( $folderPath )
{
  ...
  // Nombre del método: eliminarMateria()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Elimina una materia, tanto de la base de datos, como los ficheros
   */
  ...
  public function executeEliminarMateria()
  {
   ...
   {
      self::borrarDirectorio(sfConfig::get('sf_web_dir').'/materias/'.$this->materia->getId().'/');
      $administradores= $this->getUser()->getAdministradores();
      foreach ($administradores as $administrador)
      {  $notifacacion = new Notificacion();
         $notifacacion->setInfo($administrador->getId(),null,'Materia borrada','Materia borrada: <b>'.$this->materia->getNombre().'</b> por '.$this->getUser()->getAdministrador(),date("Y-m-d H:j"));
      }
   	  $this->materia->delete();
   	  $this->redirect('admin/materias');
   }else {//echo "tiene cursos inscritos";
  ...
  // Nombre del método: executeMatricular()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Permite matricular a un usuario en un curso tanto de profesor como de alumno
   */
  ...
  // Añadida por: Jacobo Chaquet
  /* Descripción: Muestra la ficha del curso, lo hace mediante un componente en el template
   */
  ...
  // Nombre del método: executeEliminarNotificacion()
  // Añadida por: Jacobo Chaquet
  /* Descripción: Elimina notificacion
   */
