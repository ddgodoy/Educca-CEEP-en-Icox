              ...
              <?php foreach($eventos as $evento): ?>
                  ...
                  <tr class="cont_fil" <?= $fondo1 ?>>
                      ...
                      <td class="td7"><?php if (isset($idcurso)) : ?>
                                                 <?php echo link_to('Eliminar','calendario/eliminarCitaId?idevento='.$evento->getEvento()->getId().'&idcurso='.$idcurso,'confirm=�Esta seguro que desea eliminar la cita '.$evento->getEvento()->getTitulo().' ?') ?>
                                      <?php else : ?>
												<?php echo link_to('Eliminar','calendario/eliminarCitaId?idevento='.$evento->getEvento()->getId().'&principal='.$principal,'confirm=�Esta seguro que desea eliminar la cita '.$evento->getEvento()->getTitulo().' ?') ?>
									  <?php endif; ?>
					  </td>
                  </tr>
                  <?php $i++ ?>
