...
{ 
   ...
   while ((thisAPI==null)||(thisAPI==undefined))
   {
      findAPITries++;
      ...
      if((thisAPI==null)||(thisAPI==undefined)) 
      {
		...
		if(win.document.frames)
		{
			...
			{	
				...
				if(thisFrame.document)
					thisAPI = thisFrame.document.API;
				
				if(thisFrame.API&&(thisAPI==null))
				{
					thisAPI = thisFrame.API;
				}
...
function getAPI()
{
   ...
   while((theAPI == null)&&(winActual.opener != null) && (typeof(winActual.opener) != "undefined"))
   { 
      openerCounter++;
      
      if(openerCounter>4)
      {
 	 alert("Unable to find an API adapter -- too many openers");
         return null;
      }else
      {
         theAPI = findAPI(winActual.opener);
      }
