...
switch(sExt){case"f4v":case"flv":provider="SWF"
break
case"mov":case"qt":case"mp4":case"m4v":case"m4a":case"3gp":case"3g2":if(this.MOV.installed())provider="MOV"
else if(this.SWF.installed())provider="SWF"
else provider="MOV"
break
...
return'<object classid="clsid:6BF52A52-394A-11D3-B153-00C04F79FAA6"><param name="stretchToFit" value="true"><param name="uiMode" value="none"><param name="windowlessVideo" value="'+(Providers.windowed?"false":"true")+'"></object>'
;/*@else @*/
return'<embed type="application/x-ms-wmp" stretchToFit="true" uiMode="none" windowlessVideo="'+(Providers.windowed?"false":"true")+'">'
...
;/*@else @*/
return false
;/*@end @*/},create:function(){return'<embed showcontrols="false" comptype="media" type="application/x-mplayer2" pluginspage="http://microsoft.com/windows/mediaplayer/en/download/" windowlessVideo="'+(Providers.windowed?"false":"true")+'">'},play:function(){this.engine.Play();},pause:function(){this.engine.Pause();},stop:function(){this.engine.Stop();},close:function(){this.listenPlayState(null)
this.stop()
this.setFile("")
this.engine.Cancel()
...
return'<embed comptype="flash" id="'+sId+'" src="Runtime/mplayer.swf" quality="best" type="application/x-shockwave-flash" allowFullScreen="true" pluginspage="http://www.macromedia.com/go/getflashplayer" wmode="'+(Providers.windowed?"window":"opaque")+'">'},play:function(){this._cmd("play")
...
this._fireState("play")}},MOV:{name:"MOV",installed:function(){;/*@if(@_jscript)
try{new ActiveXObject("QuickTime.QuickTime");return true;}catch(e){return false;}
;/*@else @*/
for(var iPg=0;iPg<navigator.plugins.length;iPg++)if(navigator.plugins[iPg].name.indexOf("QuickTime")>=0)return true
return false
;/*@end @*/},create:function(){return'<embed controller="false" showlogo="false" enablejavascript="true" scale="tofit" comptype="quicktime" type="video/quicktime" pluginspage="http://www.apple.com/quicktime/download/" wmode="'+(Providers.windowed?"window":"transparent")+'">'},play:function(){this.engine.Play()
...
;/*@end @*/},create:function(){return'<embed pluginspage="http://www.real.com/freeplayer/?rppr=rnwk" type="audio/x-pn-realaudio-plugin" controls="ImageWindow" scriptcallbacks="true" maintainaspect="true">'},play:function(){this.engine.DoPlay();this._fireState("play");},pause:function(){this.engine.DoPause();this._fireState("pause");},stop:function(){this.engine.DoStop();this._fireState("stop");},close:function(){this.listenPlayState(null)
this.stop()
this.setFile("")
...
this._state=state
if(this.engine.playStateHandler)this.engine.playStateHandler(oldState,state)},getTime:function(){return this.engine.GetPosition()/1000;},setTime:function(time){this.engine.SetPosition(time*1000);},isSeekable:function(){return this.engine.GetCanSeek();},getFile:function(){return this._fileName;},setFile:function(fileName){this._fileName=fileName;if(fileName)this.engine.SetSource(fileName);},getVolume:function(){return this.engine.GetVolume();},setVolume:function(pct){this.engine.SetVolume(pct);},getWidth:function(){return 320;},getHeight:function(){return 240;},getDuration:function(){return this.engine.GetLength()/1000||-1;},getAutoStart:function(){return this.engine.GetAutoStart();},setAutoStart:function(autoStart){this.engine.SetAutoStart(autoStart);if(autoStart)this._fireState("play");},getContextMenu:function(){return this.engine.GetEnableContextMenu();},setContextMenu:function(ctxMenu){this.engine.SetEnableContextMenu(ctxMenu);},listenPlayState:function(handler){clearInterval(this._intv)
if(handler){var oPrv=this,lastState=-1
this._intv=setInterval(function(){if(oPrv._state!="init"){var state=oPrv.engine.GetPlayState()
if(state==0&&lastState==0)oPrv.stop()
lastState=state}},100)}
this.engine.playStateHandler=handler}}}