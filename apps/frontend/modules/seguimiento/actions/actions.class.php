...
class seguimientoActions extends sfActions
{
     ...
     if ($this->getRequestParameter('idcurso'))
     {
  	     	...
  	     	if($curso->getMateriaId())
        	{
                   ...
                   $this->forward404Unless($materia);
                    ...
                    $c3 = new Criteria();
                    $c3->add(Sco12Peer::ID_MATERIA, $materia->getId());
                    $c3->addDescendingOrderByColumn(Sco12Peer::ID);
                    $this->scos12 = Sco12Peer::doSelect($c3);
  ...
  public function executeSource()
  {
    ...
    $idcurso = isset($data_val['idcurso'])?$data_val['idcurso']:'';
    $tipo    = isset($data_val['tipo'])?$data_val['tipo']:'';   
    $idtema  = isset($data_val['idtema'])?$data_val['idtema']:''; 
    $idsco12  = isset($data_val['idsco12'])?$data_val['idsco12']:''; 
         ...
         if($curso->getMateriaId())
      	 {
             ...
             $arrayTemas[0] ="";
              ...
              if ($materia->getTipo() == 'scorm1.2')
              {
                ...
                $c2->addAscendingOrderByColumn(Sco12Peer::ID);
  ...
  public function executeListarTareas()
  {
    ...
    $usuario = UsuarioPeer::retrieveByPk($id_profesor);
    
    $c = new Criteria();

    if ($id_curso) {$c->add(EventoPeer::ID_CURSO, $id_curso);}
    
    if(!$usuario->getInspector()){
